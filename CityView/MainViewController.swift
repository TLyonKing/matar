...
class MainViewController: UIViewController, CLLocationManagerDelegate {
	...
	// Use average of recent virthual object distances to avoid rapid changes in object scale.
...
//            print("ANCHOR UPDATE \(String(describing: planeAnchor))")
//            print("WORLD UPDATE \(String(describing: worldPos))")
		}
	}
...
//    }
     //MARK: ham nay tao buc anh 3d nhu trong demo
    func loadVirtualPlane(){
        //MARK: day la 2 vi tri fix san. Neu muon xem duoc buc anh 3d, thay 2 gia tri lat ling nay bang gia tri lat long minh dang dung (do chua lam chuc nang tu dong nhan vi tri hien tai)
        let startingLocation = CLLocation(latitude: currentlatitude!, longitude: currentlongitude!)
...
//        print("start location: ", startingLocation)
        ...
        if (mediaType == "photo" || mediaType == ""){
            sphere.addPlane1(view: contentPhoto!)
//            sphere.addNode(view: contentPhoto!)
        }else if mediaType == "video"{
//            let videoURL = URL(string: "https://d2540bljzu9e1.cloudfront.net/beta-boomtv/videos/158/Unknown_2017-07-11_12-03-16_1499749075.mp4")
            let videoURL = URL(string: self.videoURL)
            let player = AVPlayer(url: videoURL!)
         videoPlayer = rotateVideoPlayer(player: player, degrees: 90.0)
            sphere.addPlane1(view: videoPlayer!)
            player.play()
        ...
        ////
        //               }
        //                sphere.position = cameraWorldPos + cameraToPosition
    ...
    @objc func handleTap(sender: UITapGestureRecognizer) {
        ...
        if let result = hitTestResults.first {
            ...
            let location = CLLocation(latitude: latitude!, longitude:longitude!)
            //        print("start location: ", startingLocation)
            let locationTransform = MatrixHelper.transformMatrix1(for: matrix_identity_float4x4, originLocation: startingLocation, location: location)
            let sphere = BaseNode(title: "Title", location: location)
            sphere.location = location
            let stepAnchor = ARAnchor(transform: locationTransform)
            sphere.addPlane(image: UIImage(named: "chelseaFC")!)
            self.sceneView.session.add(anchor: stepAnchor)
            self.sceneView.scene.rootNode.addChildNode(sphere)
            sphere.anchor = stepAnchor
        }
...
//                    let imagePlane = SCNPlane(width: 0.16, height: 0.1)
//                    imagePlane.firstMaterial?.diffuse.contents = UIImage(named: "chelseaFC")
//
//
...
//                    var cameraToPosition = lastFocusSquarePos - cameraWorldPos
//                    cameraToPosition.setMaximumLength(self.DEFAULT_DISTANCE_CAMERA_TO_OBJECTS)
//
////                    object.position = cameraWorldPos + cameraToPosition
//                    sphereNode.position = cameraWorldPos + cameraToPosition
//
//                    if sphereNode.parent == nil {
//                        self.sceneView.scene.rootNode.addChildNode(sphereNode)
//                    }
//                    print("last posotion X: ", lastFocusSquarePos.x)
//                    print("last posotion Y: ", lastFocusSquarePos.y)
//                    print("last posotion Z: ", lastFocusSquarePos.z)
//
//                    print("cameraWorldPos X: ", cameraWorldPos.x)
//                    print("cameraWorldPos Y: ", cameraWorldPos.y)
//                    print("cameraWorldPos Z: ", cameraWorldPos.z)
//
	...
	func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
		DispatchQueue.main.async {
			if let planeAnchor = anchor as? ARPlaneAnchor {
                //MARK: ham nhan dien mat phang addPlane, chi dung cho nhan dien mat phang va tao vat tren mat phang do. Ham tao vat 3d tren mat phang la loadVirtualObject
//                self.addPlane(node: node, anchor: planeAnchor)
//                self.checkIfObjectShouldMoveOntoPlane(anchor: planeAnchor)
//                self.loadVirtualPlane()
                
                //MARK: ham loadVirtualPlane() nay duoc tao la de tao buc anh trong khong gian dua theo lat va long cua dia diem
//                self.loadVirtualPlane()
			}
		}
	}
